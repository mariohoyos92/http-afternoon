...
class User extends Component {
  ...
  componentWillMount() {
    // axios.get(`/api/users/${this.props.match.params.id}`).then((response) =>this.setState({user: response.data})).catch(console.log);
    // axios.get(`/api/blogs/${this.props.match.params.id}`).then((response) => this.setState({posts: [response.data]})).catch(console.log);
    ...
    axios
      ...
      .then(
        axios.spread((first, second) => {
          this.setState({ user: first.data });
          this.setState({ posts: [second.data] });
        })
      )
      .catch(console.log);
  }
